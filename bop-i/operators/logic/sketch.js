...
function cardTwo() {
  ...
  fill('black');
  textStyle(BOLD);
  text('Recipe 2: \n Applesauce', width/2 - (wUnit), 3*hUnit)

  push();
  textStyle(NORMAL);
  textSize(14);
  text('A recipe that includes apples', width/2 - (.75*wUnit), 6.5*hUnit)
  pop();


  textDropDown = select('#dd-2');
  textDropDown.position(width/2 - (3.25*wUnit), 11.5*hUnit);
  text('(have 0 apples)', width/2 - (.75*wUnit), 14.5*hUnit)
}
